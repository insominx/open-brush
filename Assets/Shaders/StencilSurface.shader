...
Properties {
  ...
  [Toggle]_ShowGridPoints ("Grid Points", Int) = 0
...
CGINCLUDE
  ...
  float getInteriorGrid(v2f i, in float gridSizeMultiplier, in float gridWidthMultiplier, in float UVGridWidthMultiplier) {
    ...
    if (_ShowGridPoints == 1)
    {
        float pointY = fmod((abs(localPos.y) + halfLineWidth + _GridSize / 2.0), _GridSize) < _GridLineWidth ? 1 : 0;
        float pointX = fmod((abs(localPos.x) + halfLineWidth + _GridSize / 2.0), _GridSize) < _GridLineWidth ? 1 : 0;
        interiorGrid += pointX * pointY;
    }
