...
namespace TiltBrush
{
    public struct ClassroomPanelLayouts
    {
        private const string kPlayerPrefClassroomLayout = "ClassroomLayout";

        // This is the data structure for an individual panel layout.
        private class ClassroomPanelDiskLayout
        {
        ...
        private List<ClassroomPanelDiskLayout> layouts;

        static public void ClearPlayerPrefs()
        {
            PlayerPrefs.DeleteKey(kPlayerPrefClassroomLayout);
        }
        ...
        public void PopulateFromPlayerPrefs()
        {
            ...
            if (layouts == null)
            {
                layouts = new List<ClassroomPanelDiskLayout>();
            }
            layouts.Clear();

            // Early out if we've got a blank string.
            string advLayout = PlayerPrefs.GetString(kPlayerPrefClassroomLayout, "");
            if (advLayout == "")
            {
                return;
            }
            ...
            for (int i = 0; i < advPanelLayouts.Length; ++i)
            {
                ...
                {
                    ClassroomPanelDiskLayout layout = new ClassroomPanelDiskLayout();
                    try
                    {
        ...
        public void WriteToDisk(List<PanelManager.PanelData> panels)
        {
            ...
            for (int i = 0; i < panels.Count; ++i)
            {
                ...
                // Only record the positions of non-unique, classroom panels that are showing.
                bool classroomNonUniquePanelShowing = p.m_Panel.WhiteboardLayoutPanel &&
                ...
                bool coreOrFixed = PanelManager.m_Instance.IsPanelCore(p.m_Panel.Type) ||
                    p.m_Panel.m_Fixed;
                if (classroomNonUniquePanelShowing && coreOrFixed)
                {
                    if (!first)
                    {
                    ...
                    Debug.Log($"writing {p.m_Panel.Type}");
            ...
            PlayerPrefs.SetString(kPlayerPrefClassroomLayout, layout);
        }
